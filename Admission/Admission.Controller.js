...
exports.getAllAdmission = async (req, res, next) => {
  try {
    const AdmissionData = await AdmissionModel.findAll()
...
exports.postAdmission = async (req, res, next) => {
  try {
    const value = await Schema.validateAsync(req.body)
    try {
      const savingResponse = await AdmissionModel.create(value)
      res.status(200).json({
...
exports.updateAdmission = async (req, res, next) => {
  ...
  if (!id) {
    ...
    try {
      const updateResponse = await AdmissionModel.update(req.body, {
        where: {
          id
        }
      })
...
exports.deleteAdmission = async (req, res, next) => {
  ...
  if (id) {
    try {
      const deleteResponse = await AdmissionModel.destroy({
        where: {
          id
        }
      })
